...
#def run_nrnivmodl(relative_path):
#
#    if os.path.exists("mechanisms/hippocampus/mod"):
#        command = ["nrnivmodl", relative_path + "mechanisms/hippocampus/mod"]
#    else:
#        command = ["nrnivmodl", relative_path + "mechanisms"]
#    
#    try:
...
#
#def get_hoc_morph_for_emodel_folder(relative_path):
#    """Returns the hoc morph tuple"""
#
#    # Get the single .hoc file from the electrophysiology directory
#    hoc_files = list(Path(relative_path + "electrophysiology").glob("*.hoc"))
#    if not hoc_files:
...
#    morph_files = list(Path(relative_path + "morphology").glob("*.swc")) + list(
...
def run_nrnivmodl(relative_path):
    # relative_pathをPathオブジェクトに変換
    rel_p = Path(relative_path)
    
    # パスの結合を / で行う（OSごとのスラッシュの差を自動吸収）
    mech_path = rel_p / "mechanisms" / "hippocampus" / "mod"
    if not mech_path.exists():
        mech_path = rel_p / "mechanisms"
    
    command = ["nrnivmodl", str(mech_path)]
...
def get_hoc_morph_for_emodel_folder(relative_path):
    # 文字列結合をやめ、Pathオブジェクトで処理
    base_path = Path(relative_path).resolve()

    # electrophysiology フォルダ
    electro_dir = base_path / "electrophysiology"
    hoc_files = list(electro_dir.glob("*.hoc"))
    if not hoc_files:
        raise FileNotFoundError(f"No .hoc found in {electro_dir}")
    hoc_path = hoc_files[0]

    # morphology フォルダ
    morph_dir = base_path / "morphology"
    morph_files = list(morph_dir.glob("*.swc")) + list(morph_dir.glob("*.asc"))
    if not morph_files:
        raise FileNotFoundError(f"No morphology found in {morph_dir}")
    morph_path = morph_files[0]
